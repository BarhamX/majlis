...
- Google Account through Android Credential Manager and Sign in with Apple through the system browser, using provider-required state/nonce checks and PKCE where supported.
- Explicit Google/Apple identity linking without email-based automatic merging.
- A deterministic test issuer for local development and automated tests only.
...
- Email/password, phone/SMS, guest, and social providers other than Google or Apple.
- Collection of full date of birth, phone contacts, or precise location.
- Uploaded/custom profile avatars.

## Requirements

- **AUTH-001**: Production V1 shall offer exactly two sign-in choices: Google Account through Android Credential Manager and Sign in with Apple through the system browser. Each flow shall validate provider-required state/nonce and use PKCE where supported; Majlis shall not use an embedded login webview or receive/store passwords.
- **AUTH-002**: The API shall accept only allowlisted Google or Apple identities with a valid signature, configured issuer and audience/client id, unexpired lifetime, stable subject, and verified nonce/code flow where applicable; it shall reject identity or role claims supplied in request bodies.
...
- **AUTH-010**: Account recovery and email verification shall be owned by Google or Apple. Majlis support and public deletion responses shall not reveal whether an email address or provider identity has a Majlis account.
...
- **AUTH-013**: An authenticated user and a public web flow shall support account-deletion requests. Majlis shall delete local identities, revoke provider authorization where required without deleting the person's Google/Apple account, and meet the revocation, public-removal, purge, backup-expiry, and audit-retention rules in `V1-DEC-009`.
...
- **AUTH-016**: An authenticated user may link the other supported provider only after fresh proof from both sessions. A provider identity may belong to one local user, at least one identity must remain linked, collisions shall fail safely, and identities shall never be merged by email equality.
- **AUTH-017**: A deterministic signed test issuer may be enabled only in Development and Testing. Production startup shall fail if test issuer, test signing keys, or bypass authentication is configured.

## Acceptance Criteria

- New, returning, linked-provider, expired-session, revoked-session, recovery, logout, and deletion journeys work for both Google and Apple before Production V1 release.
...
- Before `Game Ready`, local and automated flows may use only the test issuer. Google/Apple credentials and live provider tests are intentionally deferred to the post-`Game Ready` logistics phase.
